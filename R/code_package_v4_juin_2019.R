  ...
  w=w[idx]
    ...
    if(method=="edf") {
        for(s in 1:length(q)){
            ...
            up=Hmisc::wtd.quantile(x, weights=w, probs=1-p, normwt=TRUE) # weighted (1-p)-quantile
            up=as.numeric(up)

        vindex[s]=tis=sum(w*x*(x>up))/sum(w*x)
        }
    ...
    if(method=="pareto1" || method=="pareto2" || method=="gpd") {
              ...
              for(s in 1:length(q)){
        u=Hmisc::wtd.quantile(x, weights=w, probs=1-q[s], normwt=TRUE)
        ...
        if(method=="pareto1") {
            coef=MLE.pareto1(x,weights=w,threshold=u)
            sigma=u
            alpha=coef$alpha
        }
        if(method=="pareto2" || method=="gpd") {
            coef=MLE.gpd(x,weights=w,threshold=u)
        ...
        if (alpha<1) { tis=NaN
            ...
            up=Hmisc::wtd.quantile(x, weights=w, probs=1-p, normwt=TRUE)
            up=as.numeric(up)
            xp = Hmisc::wtd.mean(x[x<up], weights=w[x<up])
            den = (1-q[s])*xq + q[s]*sigma/(alpha-1) + q[s]*u
            tis = 1 - (1-p)*xp/den
        }
        vindex[s] <- tis
    ...
    for(s in 1:length(q)){
        ...
        dataqq=x[x>=u]
        coef=EPD(x[x>=u], weights=w[x>=u], direct=epd.direct)
        delta=coef$kappa
        tau=coef$tau
        alpha <- 1/coef$gamma
         xq = Hmisc::wtd.mean(x[x < u], weights = w[x < u])
        up=Hmisc::wtd.quantile(x, weights=w, probs=1-p, normwt=TRUE)
        up=as.numeric(up)
        xp = Hmisc::wtd.mean(x[x<up], weights=w[x<up])
